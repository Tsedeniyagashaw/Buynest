...
import { FiUser, FiLock,FiSettings,FiLogOut,FiChevronDown,FiShield,FiShoppingBag,FiHeart,FiGrid,FiMessageSquare,} from "react-icons/fi";
...
function UserMenu({ user }) {
  ...
  const basePath = user?.role === "admin" ? "/admin" : user?.role === "seller" ? "/seller" : "";

  if (!user) {
    return (
      <Link to="/login" className="flex items-center gap-1.5 group focus:outline-none px-2 py-1 rounded-lg hover:bg-gray-50 transition-colors" >
    ...
    <>
      <div className="relative inline-block" ref={menuRef}>
        <button onClick={() => setOpen(!open)}
          ...
          <FiChevronDown
            className={`lg:hidden w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ${ open ? "rotate-180" : ""
            }`}
          />
        </button>
        ...
        {open && (
          <div className="absolute right-0 top-full mt-2 w-64 sm:w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden animate-slideDown">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <FiShield className="w-3 h-3 text-gray-600 flex-shrink-0" />
            ...
            <div className="py-2 max-h-[calc(100vh-200px)] overflow-y-auto">
              <Link
                to={`${basePath}/profile`}
                className="flex items-center gap-3 px-4 sm:px-5 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors duration-150"
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 sm:px-5 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors duration-150"
              >
